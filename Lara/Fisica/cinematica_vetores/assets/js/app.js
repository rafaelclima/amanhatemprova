  ...
  finishExam() {
    ...
    if (container) {
      ...
      for (const [comp, stats] of Object.entries(competencies)) {
        ...
        const grade = percent >= 70 ? 'success' : 'error';
        cardsHtml += `
          <div class="ds-card" style="margin-bottom: var(--space-4);">
            <h3 style="margin: 0 0 var(--space-2) 0;">${comp}</h3>
            <p style="margin: 0 0 var(--space-2) 0;">${stats.correct}/${stats.total} acertos</p>
            <div class="ds-highlight ds-highlight--${grade === 'success' ? 'formula' : 'warning'}">${percent}% de aproveitamento</div>
          </div>`;
      }
      ...
      container.innerHTML = `
        <div class="ds-results">
          ...
          <p class="ds-score">${correct}/${exam.total} questoes corretas (${totalPercent}%)</p>
          <p>${totalPercent >= 70 ? 'Parabens! Voce mandou bem!' : 'Continue estudando para melhorar!'}</p>

          <h3 style="margin-top: var(--space-6);">Resumo por Assunto</h3>
          ${cardsHtml}
  ...
  showQuizResults() {
    ...
    container.innerHTML = `
      <div class="ds-results">
        ...
        <p class="ds-score">${quiz.score}/${quiz.total} questoes corretas (${percent}%)</p>
        <p>${percent >= 70 ? 'Parabens! Voce mandou bem!' : 'Continue estudando para melhorar!'}</p>
